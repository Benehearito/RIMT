...
@section('contentheader_description')
	Editar categoría <b>{{ $category->title_es }}</b>
@endsection
...
@section('main-content')
	<div class="row">
		<div class="col-md-12 col-lg-10 col-lg-offset-1">
			<div class="box">
				{!! Form::model($category,['route' => ['admin.categories.update', $category->id],'class'=>'form-horizontal', 'role'=>'form' , 'method' => 'PUT']) !!}
				<div class="box-header with-border">
					<h3 class="box-title">Editar categoría <b>{{ $category->title_es }}</b></h3>
					<div class="box-tools pull-right">
						<div class="btn-group">
							<button data-toggle="dropdown" class="btn btn-box-tool dropdown-toggle" type="button" aria-expanded="false">
								<i class="fa fa-wrench"></i></button>
							<ul role="menu" class="dropdown-menu">
								<li><a href="#" class="btn-delete">Eliminar categoría</a></li>
							</ul>
						</div>
				...
				<div class="box-footer">
					<p class="text-center"><button type="submit" class="btn btn-info">Actualizar categoría</button></p>
				</div>
				{!! csrf_field() !!}
				{!! Form::close() !!}
			</div>
			...
			@if (count($products)>0)
				<div class="box">
					@include('admin.categories.partials.order')
				</div>
			@endif
		</div>
	</div>
...
{{ Form::open(['route' => 'admin.imagescategories.cjt_images_saveorder' , 'method' => 'post',  'id'=>'form-ajax-saveorder-images' ]) }}
{!! Form::hidden ('category_id',$category->id ) !!}
{!! Form::hidden ('save_order_images', 0, ['id'=>'save_order_images']) !!}
{!! csrf_field() !!}
{{ Form::close() }}

{{ Form::open(['route' => 'admin.imagescategories.cjt_images_delete' , 'method' => 'delete',  'id'=>'form-ajax-delete-images' ]) }}
{!! Form::hidden ('pivot_id',0, ['id'=>'pivot_id'] ) !!}
{!! csrf_field() !!}
{{ Form::close() }}

{{ Form::open(['route' => 'admin.imagescategories.addimages' , 'method' => 'post',  'id'=>'form-ajax-addimages' ]) }}
{!! Form::hidden ('category_id',$category->id ) !!}
	...
	<script>
		$(document).ready(function (){
			...
			$('#addimagesModal').on('show.bs.modal', function (event) {
				...
				var modal = $(this)
				modal.find('.modal-title').text('Añadir imagenes a la categoria ' + recipient);
				var id= $( "#album" ).val()
				chargeImagesAdd(id);
			...
			function chargeImagesAdd (id){
				$.ajax({
					url: '{{  URL::route('admin.imagescategories.addimagescategory')  }}',
			...
			function chargeCtjoImages(){
				$('#cjto_images').fadeOut( 500, function() {
					$.ajax({
						url: '{{  URL::route('admin.imagescategories.cjt_images' , ['id' => $category->id])  }}',
						type: "GET", // not POST, laravel won't allow it
						success: function(data){
							$data = $(data); // the HTML content your controller has produced
			...
			$('#cjto_images').on('click', '.btndelete-cjtoimages', function(e){
				...
				$.confirm({
					title:"Confirmar borrado",
					text: "Esta seguro de borrar la imagen de la categoría",
					confirm: function() {
						$.post (url , data , function (result) {
							if (result.success=='ok') {
								row.fadeOut().remove();
